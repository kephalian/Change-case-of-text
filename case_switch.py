...
layout = [
#     [sg.Listbox(data, size=(max(map(len, data))+2, 15), key='LISTBOX')],
    [sg.Multiline(text1,size=(62,20), key='-LINES-', )],
    [sg.Button("Make Title", key="-TITLE-", size=(15,1)),
     sg.Button("Upper Case", key="-UPPER-",size=(15,1)),
     sg.Button("Lower case", key="-LOWER-",size=(15,1)),
     sg.Button("Clear all formatting!", key="-CLRFORMAT-",size=(15,1)),],
    [sg.Button("Copy", key="-COPY-",size=(15,1)), sg.Button("Paste", key="-PASTE-",size=(15,1)),
     sg.Button("Clear", key="-CLEAR-",size=(15,1)),],
    [sg.Button("Exit", key="-OK-",size=(50,1))], 
]
